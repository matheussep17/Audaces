Cypress.Commands.add("products_and_description", (url) => {
  ...
  cy.get('.bloco_a').realHover('mouse')
  .within(() => {
    cy.assert_contains_not_visible('O que é Audaces 360?')
    cy.assert_contains_not_visible('Solução completa em sistemas para fazer moda')
    ...
    cy.assert_contains_not_visible('Crie moldes e gradação direto no computador')
